...
as well as @Yi23a; @Yi23b; @Yi25.

## Compute Aggregated Quantiles
...
```{r}
#| code-fold: true
#| code-summary: 'A hyper data frame *`Ki67q`*: aggregated quantiles'
Ki67q |>
  head()
```

Users are encouraged to learn more about 

-   the hyper data frame (`hyperframe`) from package **`spatstat.geom`** [@Baddeley15; @Baddeley05]; 
-   each function in this pipeline in package **`groupedHyperframe`** ([CRAN](https://cran.r-project.org/package=groupedHyperframe), [Github](https://github.com/tingtingzhan/groupedHyperframe)) [vignette](https://rpubs.com/tingtingzhan/groupedHyperframe).

| Function | Purpose | Section |
|----------|------|------|
| `as.groupedHyperframe()` | to convert a data frame into a grouped hyper data frame | 2 |
| `quantile()` | to calculate the quantiles of each numeric-vector in the numeric-hypercolumn | 2 |
| `aggregate()` | to aggregate the quantiles over multiple tissues per patient by point-wise means | 2 |

: Sections in package **`groupedHyperframe`** [vignette](https://rpubs.com/tingtingzhan/groupedHyperframe)



## Estimate Integrand Surface
...
## Compute Quantile Index Predictor
...
Ki67.E = Ki67s |>
  ...
  summary_fv() |>
  aggregate(by = ~ patientID, fun = pmean)
```
...
```{r}
x0 = hyper_gam(PFS ~ logKi67.E.cumvtrapz, data = Ki67.E)
```

```{r}
x1 = hyper_gam(PFS ~ logKi67.E.cumvtrapz, data = Ki67.E, nonlinear = TRUE)
```
