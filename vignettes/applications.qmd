...
-   `r '\U1f5dd'` **`caret`** [@caret, v`r packageVersion('caret')`], key dependency, for $k$-fold prediction (to be documented in future)
-   **`cli`** [@cli, v`r packageVersion('cli')`], for attractive command line interfaces (CLIs)
-   `r '\U1f5dd'` **`mgcv`** [@mgcv, v`r packageVersion('mgcv')`], key dependency, for fitting generalized additive models
-   **`nlme`** [@nlme, v`r packageVersion('nlme')`], to import the `S3` generic function `nlme::getData()`
-   **`parallel`** shipped with `r R.version$version.string`, for parallel computing of $k$-fold prediction (to be documented in future)
-   `r '\U1f5dd'` **`plotly`** [@plotly, v`r packageVersion('plotly')`], key dependency, for interactive visualization using HTML widget

Package **`hyper.gam`** `Suggests` packages

-   **`groupedHyperframe`** [@groupedHyperframe, dev v`r packageVersion('groupedHyperframe')`], for data preparation
-   `r '\U1f5dd'` **`spatstat.geom`** [@Baddeley15, v`r packageVersion('spatstat.geom')`], for `hyperframe` data structure
...
Packages **`groupedHyperframe`** and **`hyper.gam`** require `R` version 4.5.0 (released 2025-04-11) or higher ([macOS](https://cran.r-project.org/bin/macosx/), [Windows](https://cran.r-project.org/bin/windows/base/)). An Integrated Development Environment (IDE), e.g., [RStudio](https://posit.co/download/rstudio-desktop/) or [Positron](https://positron.posit.co/download.html), is not required, but highly recommended. This vignette is created under `r R.version$version.string` using packages **`knitr`** [@knitr, v`r packageVersion('knitr')`], **`quarto`** [@quarto, v`r packageVersion('quarto')` with [Quarto](https://quarto.org/docs/get-started/) v`r quarto::quarto_version()`] and **`rmarkdown`** [@rmarkdown, v`r packageVersion('rmarkdown')`].
...
bib_hypergam =
  bibentry(
    ...
    year = '2025',
    note = ap |>
      subset.data.frame(subset = Package == 'hyper.gam', select = 'Version') |>
      c() |>
      sprintf(fmt = 'R package version %s'),
    url = 'https://CRAN.R-project.org/package=hyper.gam',
    doi = '10.32614/CRAN.package.hyper.gam'
  )
bib_groupedHyperframe =
  bibentry(
    bibtype = 'Manual',
    ...
    year = '2025',
    note = ap |>
      subset.data.frame(subset = Package == 'groupedHyperframe', select = 'Version') |>
      c() |>
      sprintf(fmt = 'R package version %s'),
...
> `r bib_groupedHyperframe |> format(style = 'text') |> gsub(pattern = '\n', replacement = ' ', x = _)`

> `r bib_hypergam |> format(style = 'text') |> gsub(pattern = '\n', replacement = ' ', x = _)`

> `r bib_QI |> format(style = 'text') |> gsub(pattern = '\n', replacement = ' ', x = _)`

<details><summary>[BibTeX](https://ctan.org/pkg/bibtex) and/or [BibLaTeX](https://ctan.org/pkg/biblatex) entries for [LaTeX](https://www.latex-project.org) users</summary>

```{r}
#| echo: false
#| comment: ''
bib_groupedHyperframe |> toBibtex()
```

```{r}
#| echo: false
#| comment: ''
bib_hypergam |> toBibtex()
```
...
Applications based on the quantile index methodology are described in our peer-reviewed publications @Yi23a; @Yi23b; @Yi25.

## Data Example

We construct a *grouped hyper data frame* object *`Ki67g`* from data example *`Ki67`* included in package **`groupedHyperframe`** ([Github](https://github.com/tingtingzhan/groupedHyperframe), [CRAN](https://CRAN.R-project.org/package=groupedHyperframe)). Grouped hyper data frame `groupedHyperframe` is an extension of the hyper data frame `hyperframe` object from package **`spatstat.geom`** [@Baddeley15; @Baddeley05]. 
...
The `groupedHyperframe` object *`Ki67g`* has 

-   A `numeric`-hypercolumn *`logKi67`*, which contains the log-transformed Ki-67 protein expression CSIs in each *`tissueID`* nested in *`patientID`*. Such nested grouping structure is denoted by *`~patientID/tissueID`* following the nomenclature of package **`nlme`** [@Pinheiro00; @nlme]. 
-   Metadata including the outcome of interest, e.g., progression free survival *`PFS`*, *`Her2`*, *`HR`*, etc. 

```{r}
#| code-fold: true
#| code-summary: 'Non-spatial `groupedHyperframe` *`Ki67g`*'
Ki67g
```


Detailed information about the `groupedHyperframe` class may be found in package **`groupedHyperframe`** vignettes ([RPubs](https://rpubs.com/tingtingzhan/groupedHyperframe), [CRAN](https://CRAN.R-project.org/package=groupedHyperframe/vignettes/groupedHyperframe.html)), section *Grouped Hyper Data Frame*.
...
@fig-integrandSurface is a collage [via package **`htmltools`**, @htmltools] of the interactive `htmlwidget` visualizations of the linear and nonlinear integrand surfaces, contours, integrand paths and their projections to the "floor" and "backwall". Users should remove the argument `n` in function call `integrandSurface(, n=21L)`, and use the default `n=501L` instead, for a more refined surface. The authors must use `n=21L` to reduce the `htmlwidget` objects size, in order to comply with `CRAN` and/or [RPubs](https://rpubs.com) file size limit.
...
```{r}
#| code-fold: true
#| code-summary: 'R code: collage multiple `htmlwidget`s'
#| label: fig-integrandSurface
#| fig-width: 5
#| fig-height: 10
#| fig-cap: 'Linear (top) and nonlinear (bottom) integrand surfaces, contours, integrand paths and their projections to the "floor" and "backwall"'
htmltools::tagList(
  m0 |> integrandSurface(n = 21L), 
  m1 |> integrandSurface(n = 21L)
) |> 
...
# 🚧 Spatial Index for Point Pattern with Continuous Marks
...
```{r}
#| code-fold: true
#| code-summary: 'Spatial `groupedHyperframe` *`Ki67s`*'
Ki67s
```
...
# 🚧 Spatial Index for Point Pattern with Multi-Type Marks
