...
Since package **`spatstat.explore`** version 3.4-3.004, we have a `Registered S3 method overwritten` when loading both packages **`groupedHyperframe`** and **`hyper.gam`**. This function name clash is between `spatstat.explore::plot.roc()` and `pROC::plot.roc()`. The package **`pROC`** [@pROC, v`r packageVersion('pROC')`] is among the `Imports` of package **`caret`** [@caret, v`r packageVersion('caret')`].
