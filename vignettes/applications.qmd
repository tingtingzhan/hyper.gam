...
@fig-integrandSurface is a collage [via package **`htmltools`**, @htmltools] of the interactive `htmlwidget` visualizations of the linear and nonlinear integrand surfaces, integrand paths and their projections to the "floor" and "backwall". Users should remove the argument `n` in function call `integrandSurface(, n=21L)`, and use the default `n=501L` instead, for a more refined surface. We must use `n=21L` to reduce the `htmlwidget` objects size, in order to comply with `CRAN` and/or [RPubs](https://rpubs.com) file size limit. 
