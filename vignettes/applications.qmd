...
Since package **`spatstat.explore`** version 3.4-3.004, we have a `Registered S3 method overwritten` when loading packages **`groupedHyperframe`** and **`hyper.gam`**. The function name clash is between `spatstat.explore::plot.roc()` and `pROC::plot.roc()`.

-   Package **`spatstat.explore`** is among the `Imports` of package **`groupedHyperframe`**
-   Package **`pROC`** [@pROC] is among the `Imports` of package **`caret`**. Package **`caret`** is among the `Imports` of package **`hyper.gam`** (for $k$-fold prediction).
